...
- (NSString *)pd_HHmmsss {
  ...
  NSDateFormatter		*formatter = [[NSDateFormatter alloc] init];
  [formatter setDateFormat:@"HHmmsss"];
...
+ (NSString*)pd_timeIntervalToHourString:(NSUInteger)min {
	...
	if (min < 60) {
		...
		return [NSString stringWithFormat:@"%@小时", @(min / 60) ];
	else
		return [NSString stringWithFormat:@"%@小时%@分钟", @(min / 60), @(min % 60)];
}
...
+ (NSString*)pd_timeIntervalToHHMMSS:(long long)oriSecond {
  ...
  if (oriSecond >= 60*60){
    ...
    [mulString appendString:[NSString stringWithFormat:@"%@:",[Utilities stringWithZero:(int)(oriSecond/(60*60))]]];
  } else{
