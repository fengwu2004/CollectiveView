...
-(NSString*) pd_substringWithRangeOfStartString:(NSString*)start endString:(NSString*)end {
	...
	int length = (int)(eRange.location - sRnage.location-sRnage.length);
	if(length<0)
		return nil;
	else
...
- (NSDate*)pd_MMMddyyyyHHmmssaDate{
  
  if (!self) {
    return nil;
  }
  
  //Oct 15, 2014 5:08:44 PM
  NSDateFormatter	*formatter = [[NSDateFormatter alloc] init];
...
- (NSUInteger) pd_wrapStringLines:(UIFont*)font withWidth:(NSUInteger)width{
  ...
  CGSize aSize = [self boundingRectWithSize:CGSizeMake(width,kMaxPixelOfCellHeight)
